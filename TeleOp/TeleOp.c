...
task main()
{
	...
	while(true) {
		...
		if(slowMode) {
			scaleFactor = -40.0/128.0;
		}
		else {
			scaleFactor = -100.0/128.0;
		}

		motor1Speed = reverse * joystick.joy1_y1 * scaleFactor;
		motor2Speed = reverse * joystick.joy1_y2 * scaleFactor;
		if(reverse == 1){
			motor[leftMotors] = motor1Speed;
			motor[rightMotors] = motor2Speed;
		}
		else {
			motor[leftMotors] = motor2Speed;
			motor[rightMotors] = motor1Speed;
		}
	}
}
